...
import { PolygonQueryResponse, PriceQueryResponse, PolygonPriceQueryResponse } from './price-query.type';
...
export class PriceQueryEffects {
    ...
    PriceQueryActionTypes.FetchPriceQuery,
    {
      run: (action: FetchPriceQuery, state: PriceQueryPartialState) => {
        return this.httpClient.get(this.buildUrl(action)).pipe(
          map(resp => {
            return new PriceQueryFetched(this.convertPriceQuery((resp as PolygonQueryResponse).results));
          })
        );
      },
  ...
  convertPriceQuery(response: PolygonPriceQueryResponse[]): PriceQueryResponse[] {
